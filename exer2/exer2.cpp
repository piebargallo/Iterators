/* Write a program that creates a vector of pairs, where each pair contains an instance of the person class introduced
   earlier in this chapter, and their age. Next, use the ranges library to construct a sinple pipeline that extracts
   all pages from all persons from the vector, and removes all ages below 12 and above 65. Finally, calculate the
   average of the remaining ages using the sum() algorithm from earlier in this chapter. As you´ll pass a range to the
   sum() algoritm, you´ll have to work with a common range. */
